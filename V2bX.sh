...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/baimi-bm/V2bX-script/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/baimi-bm/V2bX-script/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
wget -O /usr/bin/V2bX -N --no-check-certificate https://raw.githubusercontent.com/baimi-bm/V2bX-script/main/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/baimi-bm/testv2bx ---
  ${green}0.${plain} 修改配置
————————————————
